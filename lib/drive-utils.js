...
export async function uploadFileToGoogleDrive({ bytes, filename, mimeType = 'application/octet-stream', token }) {
  const metadata = {
    name: filename,
    mimeType
  };
...
export async function uploadCodexEntryToGoogleDrive({ entry, token, uploadFileFn }) {
