...
export function showError(statusDiv, message, recovery) {
  if (typeof statusDiv === "string") {
    console.error(
      "showError called with string instead of DOM element:",
      statusDiv,
    );
    return;
  }
...
export async function validateAndShowEntry(
  statusDiv,
  certificateSummary,
  response,
) {
...
export function updateCodexUI(
  ...
  // Show createdBy info
  if (response.entry.createdBy) {
    let creatorInfo = "Created By: ";
    if (response.entry.createdBy.type === "google") {
      creatorInfo +=
        response.entry.createdBy.name || response.entry.createdBy.email;
      if (response.entry.createdBy.picture) {
        creatorInfo += ` <img src="${response.entry.createdBy.picture}" alt="profile" style="height:1.5em;vertical-align:middle;border-radius:50%;">`;
      }
    } else {
      creatorInfo += response.entry.createdBy.type;
    }
    certificateSummary.textContent += `\n${creatorInfo}`;
  }
  ...
  if (
    ...
    response.entry.storage.location &&
    response.entry.storage.location.startsWith("https://drive.google.com/") &&
    payloadExists
  ) {
