...
describe('zip-archive', () => {
  ...
  const testPassword = 'test-password'; // Password for testing
  ...
  test('creates a valid zip archive with payload and codex entry', async () => {
    ...
    // Read the zip and verify contents (with password)
    const zip = await JSZip.loadAsync(zipBlob, { password: testPassword });
    ...
    // Verify codex-entry.json content (storage.location, anchor.tx, anchor.url should be excluded)
    ...
    expect(codexJson.anchor.tx).toBeUndefined();
    expect(codexJson.anchor.url).toBeUndefined();
  ...
  test('sets archive-level comment to full codex entry', async () => {
    ...
    // Read the zip and check the comment (with password)
    const zip = await JSZip.loadAsync(zipBlob, { password: testPassword });
    ...
    // Verify that the comment INCLUDES storage.location, anchor.tx, anchor.url (full entry)
    expect(commentJson.storage.location).toBe(
      'https://drive.google.com/file/d/abc123'
    );
    expect(commentJson.anchor.tx).toBe('tx123');
  });

  test('throws error if payloadBytes is not a Uint8Array', async () => {
    await expect(
      createCodexZipArchive('invalid', samplePayloadFilename, sampleCodexEntry, testPassword)
    ).rejects.toThrow('payloadBytes must be a Uint8Array');
  });

  test('throws error if payloadFilename is not a string', async () => {
    await expect(
      createCodexZipArchive(samplePayloadBytes, null, sampleCodexEntry, testPassword)
    ).rejects.toThrow('payloadFilename must be a non-empty string');
  });

  test('throws error if codexEntry is not an object', async () => {
    await expect(
      createCodexZipArchive(samplePayloadBytes, samplePayloadFilename, null, testPassword)
    ).rejects.toThrow('codexEntry must be an object');
  });

  test('throws error if password is not provided', async () => {
    await expect(
      createCodexZipArchive(samplePayloadBytes, samplePayloadFilename, sampleCodexEntry, null)
  ...
  test('preserves all codex entry fields except storage.location, anchor.tx, anchor.url', async () => {
    const zipBlob = await createCodexZipArchive(
      samplePayloadBytes,
      samplePayloadFilename,
      sampleCodexEntry,
    ...
    // Check all fields are present except storage.location, anchor.tx, anchor.url
    expect(codexJson.id).toBe(sampleCodexEntry.id);
    ...
    expect(codexJson.anchor.chain).toBe(sampleCodexEntry.anchor.chain);
    expect(codexJson.anchor.hash_alg).toBe(sampleCodexEntry.anchor.hash_alg);
    expect(codexJson.anchor.tx).toBeUndefined();
    expect(codexJson.anchor.url).toBeUndefined();
